        ...
        for (int index = 0, count = 0; index < length; index++, count++) {
            ...
            for (String announcement : ITEMS_ANNOUNCED) {
                if (name.contains(announcement)) {
                    World.sendMessage("<icon=12><col=5739B3> OSR: <col=" + player.right.getColor() + ">" + player.getName() + " </col>has received " + Utility.getAOrAn(name) + " <col=5739B3>" + name + " </col>from <col=5739B3>" + item.getName().toLowerCase() + "</col>.");
                    Discord.message(player.getName() + " has just received " + Utility.getAOrAn(name) + " " + name + " from " + item.getName().toLowerCase() + "!");
                }
            }
        }
