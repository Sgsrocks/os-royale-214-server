...
public class StarterKitButtonPlugin extends PluginContext {
    ...
    private static void confirm(Player player) {
        if (player.buttonDelay.elapsed(1, TimeUnit.SECONDS)) {
            ...
            player.message("<col=3559a5>RPK: </col>You will now be playing as " + Utility.getAOrAn(name) + " <col=3559a5>" + name + "</col> player.");
            player.message("<col=3559a5>RPK: </col>If you have any questions please visit our forums or contact a staff member.");
            player.locking.unlock();
            player.buttonDelay.reset();
            player.interfaceManager.close();

            PlayerSerializer.save(player);
            ClanChannelHandler.connect(player, "runepk");
            World.sendStaffMessage("<col=FF0000>" + player.getName() + " is a new player and has just joined! Make him feel welcomed!!!");
        }
    }
}
