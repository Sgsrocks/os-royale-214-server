...
public class Pets {
    ...
    public static boolean onSpawn(Player player, int item, boolean drop) {
        if (!PetData.forItem(item).isPresent())
            return false;
        if (Area.inEventArena(player) || player.getY() == 3523 || player.getY() == 3524) {
        ...
        if (!drop) {
            ...
            World.sendMessage("<icon=11> <col=FF0000>RPK: <col=" + player.right.getColor() + ">" + player.getName() + "</col> has just received a pet " + pet.getName() + "!");
