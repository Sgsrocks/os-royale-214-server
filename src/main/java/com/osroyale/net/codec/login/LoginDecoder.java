...
import com.osroyale.util.ByteBufUtils;
    ...
    private void decodePayload(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (in.isReadable(loginBlockSize)) {
            ...
            if (Config.CLIENT_VERSION != clientVersion) {
                logger.warn("[%" + host + "] outdated client(client version): %" + clientVersion + " should be: %" + Config.CLIENT_VERSION + "");
                sendResponseCode(ctx, LoginResponse.GAME_UPDATED);
                return;
            }

            final int cacheVersion = in.readUnsignedByte();
            if (Config.CACHE_VERSION != cacheVersion) {
                logger.warn("[%" + host + "] outdated client(cache version): %" + cacheVersion + " should be: %" + Config.CACHE_VERSION + "");
                sendResponseCode(ctx, LoginResponse.GAME_UPDATED);
                return;
            }

            final String gameVersion = ByteBufUtils.readString(in);
            if (!gameVersion.equalsIgnoreCase(Config.GAME_VERSION)) {
                logger.warn("[%" + host + "] outdated client(game version): %" + gameVersion + " should be: %" + Config.GAME_VERSION + "");
                sendResponseCode(ctx, LoginResponse.GAME_UPDATED);
                return;
            }
