...
public enum PlayerRight {
    ...
    MANAGER("Manager", "D17417", 2, -1, 4116),
    ...
    public static boolean isDonator(Player player) {
        return isModerator(player) || player.donation.getSpent() >= DONATOR.getMoneyRequired();
    }

    /** Checks if the player has super donator status. */
    public static boolean isSuper(Player player) {
        return isModerator(player) || player.donation.getSpent() >= SUPER_DONATOR.getMoneyRequired();
    }
