...
public class TzTokJad extends MultiStrategy {
    ...
    private static class MeleeAttack extends NpcMeleeStrategy {
        ...
        public CombatHit[] getHits(Npc attacker, Mob defender) {
            return new CombatHit[] { nextMeleeHit(attacker, defender, 97, 2, 2) };
        }
    }
    ...
    private static class RangedAttack extends NpcRangedStrategy {
        ...
        @Override
        public void hit(Npc attacker, Mob defender, Hit hit) {
            defender.graphic(new Graphic(451, UpdatePriority.HIGH));
        ...
        public void hitsplat(Npc attacker, Mob defender, Hit hit) {
            defender.graphic(new Graphic(157, UpdatePriority.HIGH));
            if (defender.prayer.isActive(Prayer.PROTECT_FROM_RANGE)) {
                hit.modifyDamage(damage -> 0);
            }
        }
    ...
    private static class MagicAttack extends NpcMagicStrategy {
        ...
        public void hit(Npc attacker, Mob defender, Hit hit) {
            ...
            World.sendProjectile(attacker, defender, new Projectile(448, 10, 45, 130, 31));
            World.sendProjectile(attacker, defender, new Projectile(449, 17, 45, 130, 31));
            World.sendProjectile(attacker, defender, new Projectile(450, 22, 45, 130, 31));
        }

        @Override
        public void hitsplat(Npc attacker, Mob defender, Hit hit) {
            defender.graphic(new Graphic(157, true, UpdatePriority.HIGH));
