...
import com.osroyale.net.packet.out.SendSkill;
    ...
    private void updateSkill(Player player, int maxLevel, int newMax, Skill skill, boolean levelUp, boolean combat) {
        if (maxLevel < 99 && newMax != maxLevel) {
            ...
            if (levelUp) {
                ...
                if (newMax == 99 && !combat) {
                    World.sendMessage(
                            "<col=7B44B3>OSR: <col=" + player.right.getColor() + ">" + player.getName() + " </col>has just reached level 99 in <col=7B44B3>" + Skill.getName(skill.getSkill()) + "</col>!");
                    Discord.message(player.getName() + " has just reached level 99 in " + Skill.getName(skill.getSkill()) + "!");
                }
            } else {
